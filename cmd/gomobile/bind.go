...
import (
	...
	"github.com/Sidetalker/mobile/internal/importers/objc"
)
...
func (b *binder) GenGoSupport(outdir string) error {
	bindPkg, err := ctx.Import("github.com/Sidetalker/mobile/bind", "", build.FindOnly)
	if err != nil {
		return err
	}
...
func (b *binder) GenObjcSupport(outdir string) error {
	objcPkg, err := ctx.Import("github.com/Sidetalker/mobile/bind/objc", "", build.FindOnly)
	if err != nil {
		return err
	}
...
func (b *binder) GenJavaSupport(outdir string) error {
	javaPkg, err := ctx.Import("github.com/Sidetalker/mobile/bind/java", "", build.FindOnly)
	if err != nil {
		return err
	}
